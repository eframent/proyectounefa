...
class  cls_ProyeccionSec extends  cls_Bitacora{
	...
	public function f_Carga($variable){
		...
		$ls_Sql="SELECT  p.codesp,c.nombre AS carrera,p.regimen,r.tipo_turno,p.semestre ,s.nombre AS sem,p.seccion,sec.nombre AS sec,
				...
				AND (p.codesp=(SELECT codesp FROM carrera WHERE nombre LIKE '%$variable%')) 
				AND (p.semestre<>'0')
	...
	public function f_Maquetar($variable){
		...
		echo "<div tituloTrans> <label class='titulo'>PROYECCION DE SECCIONES <T><L>Carrera:</L>".$la_MatrizCons[0]." <L>Periodo Academico:</L>".$la_MatrizCons[1]."</label></T> </div>";
		for($x=0;$x<count($this->aa_Semestres);$x++){			
			...
			echo"<div nosequiees onclick='reducir(this)' style='box-shadow:0px 0px 2px black;' ".$x."> 
				...
				$HTMLNocturnas="<div class='contentField reducir' nocturnas><div legenda>Secciones Nocturnas</div>";
							...
							if($controlSecciones!=$la_Matriz[$y][8]){
								...
								if($la_Matriz[$y][2]=="1")//nocturna
								{
									if($contadorSeccionesN!=0){
										$HTMLNocturnas.="</article><article><div class='K' style='text-transform: capitalize;'>Pulse Una Materia Para Ver Sus Detalles</div></article></div>";
									}
									$HTMLNocturnas.="<div class='legend' onclick='controlField(this)'>".$la_Matriz[$y][7]."</div>";
										$HTMLNocturnas.="<div class='fieldset'><input class='ModificarSec' type='button' value='Modificar' onclick='modificarSec(\"".$la_Matriz[$y][8]."\")'><input type='button' value='Eliminar' onclick='eliminarSeccion(\"".$la_Matriz[$y][8]."\")' class='eliminarSec'>";
								...
								else if($la_Matriz[$y][2]=="2")//diurna
								{
									if($contadorSeccionesD!=0){
										$HTMLDiurnas.="</article><article><div class='K' style='text-transform: capitalize;'>Pulse Una Materia Para Ver Sus Detalles</div></article></div>";
									}
									$HTMLDiurnas.="<div class='legend' onclick='controlField(this)'>".$la_Matriz[$y][7]."</div>";
										$HTMLDiurnas.="<div class='fieldset'><input class='ModificarSec' type='button' value='Modificar' onclick='modificarSec(\"".$la_Matriz[$y][8]."\")' ><input value='Eliminar' type='button' onclick='eliminarSeccion(\"".$la_Matriz[$y][8]."\")' class='eliminarSec'>";
					...
					$controlCreada=0;
				$HTMLNocturnas.="<div class='ultimoLeg legend' title='Crear Seccion' nocturnas ";
				$HTMLDiurnas.="<div class='ultimoLeg legend' title='Crear Seccion' diurnas ";
				if($la_MatrizCons[0]!=""){
					$HTMLNocturnas.="onclick='Crear(\"".($seccionAcrearN)."\",\"".$la_MatrizCons[2]."\",\"".$this->aa_Semestres[$x][0]."\",\"".$la_MatrizCons[3]."\",\"1\")'";
					$HTMLDiurnas.="onclick='Crear(\"".($seccionAcrearD)."\",\"".$la_MatrizCons[2]."\",\"".$this->aa_Semestres[$x][0]."\",\"".$la_MatrizCons[3]."\",\"2\")'";
				}
				$HTMLDiurnas.="></div></div>";
				$HTMLNocturnas.="></div></div>";				
				print($HTMLDiurnas);
				print($HTMLNocturnas);
		}
	}
