...
class  cls_Depuracion extends  cls_Bitacora{
	...
	/**********************************************************************************************************************/
		public function f_BuscarMaterias(){																							
			...
			$liI=0;
			$lb_Enc=false;
			$numeroIns=$this->aa_Form['Ins'];
			$peraca=$_SESSION['peraca'];
			$ls_Sql="SELECT m.codmat,m.nommat,pm.pm_codigo,s.nombre AS seccion,r.tipo_turno AS turno,pm.tope AS tope,count(ins.cedula_est_pre) AS estudiantes
					FROM planificacion_materias AS pm
					...
					LEFT JOIN inscripcion_pre AS ins ON(pm.pm_codigo=ins.pm_codigo)
					WHERE pm.codmat=(
							SELECT pm.codmat 
							FROM inscripcion_pre AS ins 
							INNER JOIN planificacion_materias AS pm ON(pm.pm_codigo=ins.pm_codigo)
							WHERE ins.num_inscripcion='$numeroIns'
						)
					AND p.peraca='$peraca'
					AND pm.pm_codigo<>(
							SELECT pm.pm_codigo
					...
					GROUP BY m.codmat,m.nommat,pm.pm_codigo,pm.tope,s.nombre,r.tipo_turno,ins.peraca
			...
			while($la_Tupla=$this->f_Arreglo($lr_Tabla)){																		
				...
				$materias[$liI]['Tope']=$la_Tupla['tope'];	
				$materias[$liI]['Estudiantes']=$la_Tupla['estudiantes'];	
				$liI++;
				$lb_Enc=true;																								
			}
